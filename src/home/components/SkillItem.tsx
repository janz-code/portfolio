...
export default function CategoryContainer({category, children, locale, items}:Props){
    ...
    return (
        <li className="border-2 border-light-border px-2 py-1 sm:px-4 sm:py-2 rounded-2xl flex flex-col gap-y-3 h-fit">
            <h4 className="capitalize text-base sm:text-xl 2xl:text-3xl font-cascadia font-semibold">{category.name}</h4>
            <ul className={`2xl:px-3 px-1.5 2xl:py-1 py-0.5 grid 2xl:gap-y-3 lg:gap-y-1.5 list-none
                ${active ? "h-fit" : "h-[290px]"}
